...
\begin{document}
    ...
    you closer to your reader!

    \section{Quotes, and Emphasis}
    Note that double  "quotes" are not typeset  correctly.  English quotes 
    should  look  like ``this'',  while  German  quotes are  typeset  like 
    "`this"'.   Note  the difference.   Some  editors  insert the  correct 
    ...
    Don't use  colloquial expressions.  In  particular, do not use  any of 
    the short  forms \emph{don't,  aren't, isn't, you're,  we're, \ldots}. 
    Do not\footnote{this is better!} use ellipsis (\ldots) in the text, as 
    it looks like you're trailing off.                                     
    ...
    left/right and center region should  have equal widths.  The margin on 
    the  bottom  should be  larger  than  on  the  top.  These  rules  are 
    automatically  used  by  the KomaScript  classes  (scrartcl,  scrbook, 
    scrrprt).                                                              
    ...
    space  for it  using  BCOR.  It  allows a  correct  typography of  the 
    document.  Never, ever, use the \verb+geometry+ package.
    ...
    Paragraphs  are  separated  by  empty  lines.   Never  use  the  double
    backslash for this purpose.
    ...
    lines will add  space and indentation.  So  if a formula is  part of a 
    paragraph, do not add empty lines around it. 
    ...
    Note  that tables  generally have  their caption  above the  table, not
    below (as in figures).
    ...
    If you know what you're doing you can get away with less.
